...
public class GoStat extends SQLiteOpenHelper {
    ...
    public String getWinString(int win[], String eol) {
        ...
        if (total == 0) {
            ...
            if (win[0] == total)
                return String.format(Locale.US, " %5d  - %3d  - %5d%s   %3.0f%% - %3.0f%% -  %4.1f%%", win[0], win[2], win[1], eol, pro[0], pro[2], pro[1]);
            if (win[1] == total)
                return String.format(Locale.US, " %5d  - %3d  - %5d%s  %4.1f%% - %3.0f%% -   %3.0f%%", win[0], win[2], win[1], eol, pro[0], pro[2], pro[1]);
            if (win[2] == total)
                return String.format(Locale.US, " %5d  - %3d  - %5d%s  %4.1f%% -   %3.0f%% -  %4.1f%%", win[0], win[2], win[1], eol, pro[0], pro[2], pro[1]);
    ...
    private class PlayerPair {
        ...
        public void add(int rules, int board, int aMark, int win, int val) {
            ...
            if (res[rules][board][startIndex][winIndex] < 0)
                res[rules][board][startIndex][winIndex] = 0;
        }
                ...
                for (int iBoard = 0; iBoard < GoSettings.numBoards; iBoard += 1) {
                    for (int iStart = 0; iStart < 2; iStart += 1) {
                        for (int iWin = 0; iWin < 3; iWin += 1) {
                            ...
                            if (res[iRules][iBoard][iStart][iWin] < 0)
                                res[iRules][iBoard][iStart][iWin] = 0;
                        }
                    }
                }
