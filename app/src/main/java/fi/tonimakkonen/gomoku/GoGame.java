...
public class GoGame implements Cloneable {
	...
	private void addNeighbor(int ti, int val) {
		...
		if(gameBoard == 0) {
			...
			int x1 = x + 1; if(x1 >= GoSettings.goNormalSide) x1 = GoSettings.goNormalSide - 1;
			int y0 = y - 1; if(y0 < 0) y0 = 0;
			int y1 = y + 1; if(y1 >= GoSettings.goNormalSide) y1 = GoSettings.goNormalSide - 1;
				...
				for(int dir = 0; dir < dirNum; dir++) {
					...
					for(int dist = 1; dist < 6; dist ++) {
						...
						if(neighbor1[index] > 0 && status[index] == 0 && used[index] == 0) {
							if(loopInd == 0) {used[index] = 1; count += 1; }
							else { used[index] = 1; retval[count] = index; count += 1; }
						}
			...
			if(size > 0) {
				for(int i = 0; i < GoSettings.goMaxSize; i++) {
					if(neighbor1[i] > 0 && status[i] == 0 && used[i] == 0) {
						if(loopInd == 0) { used[i] = 1; count += 1; }
						else { used[i] = 1; retval[count] = i; count += 1; }
					}
				}
			}
